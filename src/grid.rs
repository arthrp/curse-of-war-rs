...
fn x_of_ij(i: i32, j: i32) -> f32 {
    0.5 * (j as f32) + (i as f32)
}
fn y_of_ij(_i: i32, j: i32) -> f32 {
    j as f32
}
...
pub fn apply_stencil(
    ...
    g: &mut Grid,
    d: i32,
    loc: &mut [Loc; MAX_AVLBL_LOC],
    avlbl_loc_num: &mut i32,
) {
...
fn floodfill_closest(
    g: &Grid,
    u: &mut [[i32; MAX_HEIGHT]; MAX_WIDTH],
    d: &mut [[i32; MAX_HEIGHT]; MAX_WIDTH],
    x: i32,
    y: i32,
    val: i32,
    dist: i32,
    ...
    if x < 0
        ...
        || y < 0
        || y >= g.height
        || !is_inhabitable(g.tiles[x as usize][y as usize].cl)
...
fn eval_locations(g: &Grid, loc: &[Loc], result: &mut [i32], len: usize) {
    ...
    for i in 0..g.width as usize {
        for j in 0..g.height as usize {
            d[i][j] = (MAX_WIDTH * MAX_HEIGHT + 1) as i32;
            u[i][j] = -1;
        }
    }
    for k in 0..len {
        floodfill_closest(g, &mut u, &mut d, loc[k].i, loc[k].j, k as i32, 0);
    }
    for i in 0..g.width as usize {
        for j in 0..g.height as usize {
            if g.tiles[i][j].cl == TileClass::Mine {
                let mut single_owner = -1;
                let mut max_dist = 0;
                let mut min_dist = (MAX_WIDTH * MAX_HEIGHT + 1) as i32;
                for k in 0..DIRECTIONS {
                    let x = i as i32 + DIRS[k].i;
                    let y = j as i32 + DIRS[k].j;
                    if x < 0
                        || x >= g.width
                        || y < 0
                        || y >= g.height
                    ...
                    if single_owner == -1 {
                        single_owner = u[x as usize][y as usize];
                        max_dist = d[x as usize][y as usize];
                        min_dist = d[x as usize][y as usize];
                    } else {
                        if u[x as usize][y as usize] == single_owner {
                            max_dist = max_i32(max_dist, d[x as usize][y as usize]);
                            min_dist = min_i32(min_dist, d[x as usize][y as usize]);
                        } else if u[x as usize][y as usize] != -1 {
                            single_owner = -2;
                        }
                    }
                }
                if single_owner != -2 && single_owner != -1 {
                    result[single_owner as usize] += (100.0
                        ...
                        * (-10.0 * (max_dist as f32) * (min_dist as f32)
                            / ((MAX_WIDTH * MAX_HEIGHT) as f32))
                            .exp()) as i32;
                }
            }
...
fn shuffle(arr: &mut [i32]) {
    let mut rng = rand::thread_rng();
    for _ in 0..arr.len() {
        let i = rand::Rng::gen_range(&mut rng, 0..arr.len());
        let j = rand::Rng::gen_range(&mut rng, 0..arr.len());
        arr.swap(i, j);
    }
}
...
pub fn conflict(
            ...
            for p in 0..MAX_PLAYER {
                for c in 0..MAX_CLASS {
                    ...
                    if is_a_city(g.tiles[i][j].cl) {
                        g.tiles[i][j].cl = TileClass::Grassland;
                    }
                }
            }
        }
    ...
    let di = rand::Rng::gen_range(
        &mut rand::thread_rng(),
        0..available_loc_num.max(1) as usize,
    );
    let mut chosen_loc = [Loc { i: 0, j: 0 }; MAX_AVLBL_LOC];
    let mut i = 0usize;
    while i < num {
        let ii = (i + di) % (available_loc_num as usize);
        let x = loc_arr[ii].i;
        let y = loc_arr[ii].j;
        chosen_loc[i] = Loc { i: x, j: y };
        g.tiles[x as usize][y as usize].cl = TileClass::Castle;
        let dir = rand::Rng::gen_range(&mut rand::thread_rng(), 0..DIRECTIONS);
        let ri = DIRS[dir].i;
        let rj = DIRS[dir].j;
        let m = 1;
        let mut mine_i = x + m * ri;
        let mut mine_j = y + m * rj;
        g.tiles[mine_i as usize][mine_j as usize].cl = TileClass::Mine;
        g.tiles[mine_i as usize][mine_j as usize].pl = NEUTRAL as i32;
        mine_i = x - 2 * m * ri;
        mine_j = y - 2 * m * rj;
        g.tiles[mine_i as usize][mine_j as usize].cl = TileClass::Mine;
        g.tiles[mine_i as usize][mine_j as usize].pl = NEUTRAL as i32;
        mine_i = x - m * ri;
        mine_j = y - m * rj;
        g.tiles[mine_i as usize][mine_j as usize].cl = TileClass::Grassland;
        g.tiles[mine_i as usize][mine_j as usize].pl = NEUTRAL as i32;
        i += 1;
    }
    let mut eval_result = [0i32; MAX_AVLBL_LOC];
    let mut loc_index = [0i32, 1, 2, 3, 4, 5, 6];
    eval_locations(g, &chosen_loc, &mut eval_result, num);
    sort_increasing(&mut eval_result, &mut loc_index, num);
    if ineq != RANDOM_INEQUALITY {
        let mut avg = 0f32;
        for i in 0..num {
            avg += eval_result[i] as f32;
        }
        avg /= num as f32;
        let mut var = 0f32;
        for i in 0..num {
            var += (eval_result[i] as f32 - avg).powi(2);
        }
        var /= num as f32;
        let diff = var.sqrt();
    ...
    shuffle(&mut sh_players_comp);
    let mut sh_players = vec![0i32; num];
    let mut idx = 0usize;
    while idx < ui_players_num as usize {
        sh_players[idx] = ui_players[idx];
        idx += 1;
    }
    let dplayer = rand::Rng::gen_range(&mut rand::thread_rng(), 0..players_num.max(1) as usize);
    while idx < num {
        sh_players[idx] =
            sh_players_comp[(idx - ui_players_num as usize + dplayer) % (players_num as usize)];
        idx += 1;
    }
    shuffle(&mut sh_players);
    let mut ihuman = rand::Rng::gen_range(&mut rand::thread_rng(), 0..num);
    if conditions > 0 {
        let select = in_segment(num as i32 - conditions, 0, num as i32 - 1) as usize;
        ihuman = loc_index[select] as usize;
    }
    for i in 0..num {
        let ii = loc_index[i] as usize;
        let x = chosen_loc[ii].i as usize;
        let y = chosen_loc[ii].j as usize;
        if ui_players_num > 1 {
        ...
        g.tiles[x][y].units[g.tiles[x][y].pl as usize][UnitClass::Citizen as usize] = 10;
    }
    0
}
...
pub fn is_connected(g: &Grid) -> bool {
    let mut m = [[0i32; MAX_HEIGHT]; MAX_WIDTH];
    let mut colored = false;
    for i in 0..g.width as usize {
        for j in 0..g.height as usize {
            if g.tiles[i][j].pl != NEUTRAL as i32 {
                if colored && m[i][j] == 0 {
                    return false;
                }
                colored = true;
                floodfill(g, &mut m, i as i32, j as i32, 1);
            }
        }
    }
    true
...
pub fn spread(
    g: &Grid,
    u: &mut [[i32; MAX_HEIGHT]; MAX_WIDTH],
    v: &mut [[i32; MAX_HEIGHT]; MAX_WIDTH],
    x: i32,
    y: i32,
    val: i32,
    factor: i32,
...
pub fn even(g: &Grid, v: &mut [[i32; MAX_HEIGHT]; MAX_WIDTH], x: i32, y: i32, val: i32) {
    if x < 0 || x >= g.width || y < 0 || y >= g.height || v[x as usize][y as usize] == val {
        return;
    }
